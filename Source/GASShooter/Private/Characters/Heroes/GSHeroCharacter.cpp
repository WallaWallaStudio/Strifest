...
AGSHeroCharacter::AGSHeroCharacter(const class FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	bStartInFirstPersonPerspective = false;
...
void AGSHeroCharacter::SetPrimaryReserveAmmo(int32 InAmmoCount)
{
	if (AmmoAttributeSet)
	{
		AmmoAttributeSet->SetPistolReserveAmmo(InAmmoCount);
	}
}
...
bool AGSHeroCharacter::CanSprint_Implementation() const
{
	return false;
}

bool AGSHeroCharacter::CanAim_Implementation() const
{
	return false;
}

void AGSHeroCharacter::SetMouseSensitivity(float InX, float InY)
{
	...
	if (!IsValid(PC->PlayerInput))
	{
		return;
	}

	PC->PlayerInput->SetMouseSensitivity(InX, InY);
}
...
void AGSHeroCharacter::MoveForward(float Value)
{
	if (IsAlive())
	{
		FVector ForwardVector =  UKismetMathLibrary::GetForwardVector(FRotator(0,
			GetControlRotation().Yaw + MoveForwardOffset.Y, 0));
...
void AGSHeroCharacter::MoveRight(float Value)
{
	if (IsAlive())
	{
		AddMovementInput(UKismetMathLibrary::GetRightVector(FRotator(0, GetControlRotation().Yaw + MoveForwardOffset.Y, 0)), Value);
	}
}
...
UCameraComponent* AGSHeroCharacter::GetFollowCamera_Implementation() const
{
	return nullptr;
}
