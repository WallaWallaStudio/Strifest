...
void AGSPlayerController::CreateHUD()
{
	...
	UIHUDWidget->SetWood(PS->GetWood());
...
void AGSPlayerController::SetHitImpactForReticle()
{
	if (!IsValid(UIHUDWidget))
	{
		return;
	}
	
	UGSHUDReticle* Reticle = UIHUDWidget->GetReticleWidget();
	if (!IsValid(Reticle))
	{
		return;
	}
	
	Reticle->FireHitImpact();
}
