...
void AGASShooterGameModeBase::BeginPlay()
{
	...
	{
		if (Actor->GetActorNameOrLabel() == FString("EnemyHeroSpawn"))
		{
...
void AGASShooterGameModeBase::RespawnHero(AController* Controller)
{
	...
	else
	{
		...
		TSubclassOf<class AGSHeroCharacter> AIHeroClass = StaticLoadClass(UObject::StaticClass(), nullptr, TEXT("/Game/GASShooter/AI/BP_AI.BP_AI_C"));
		
		AGSHeroCharacter* Hero = GetWorld()->SpawnActor<AGSHeroCharacter>(AIHeroClass, EnemySpawnPoint->GetActorTransform(), SpawnParameters);
